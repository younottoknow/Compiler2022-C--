...
SymbolTableEntry* enterSymbol(char* symbolName, SymbolAttribute* attribute)
{
    ...
    while(hashChain)
    {
        if(strcmp(hashChain->name, symbolName) == 0)
        {
            ...
            {
                removeFromHashChain(hashIndex, hashChain);
    ...
    while(hashChain)
    {
        if(strcmp(hashChain->name, symbolName) == 0)
        {
            ...
            else
            {
                ...
                {
                    enterIntoHashChain(hashIndex, hashChain->sameNameInOuterLevel);
                }
                break;
            }
        }
...
void closeScope()
{
    ...
    while(scopeChain)
    {
        ...
        removeFromHashChain(hashIndex, scopeChain);
        if(scopeChain->sameNameInOuterLevel)
        {
            enterIntoHashChain(hashIndex, scopeChain->sameNameInOuterLevel);
        }
        scopeChain = scopeChain->nextInSameLevel;
    }
