...
static inline AST_NODE* makeConstNode(CON_Type* C)
{
...
factor		: MK_LPAREN relop_expr MK_RPAREN
            ...
            | OP_PLUS ID MK_LPAREN relop_expr_list MK_RPAREN
                {
                    $$ = makeExprNode(UNARY_OPERATION, UNARY_OP_POSITIVE);
                    AST_NODE* function_call = makeStmtNode(FUNCTION_CALL_STMT);
                    makeFamily(function_call, 2, makeIDNode($2, NORMAL_ID), $4);
                    makeChild($$, function_call);
                }
...
char *argv[];
{
    yyin = fopen(argv[1],"r");
    yyparse();
    // if (argc == 2) printGV(prog, NULL);
    // else printGV(prog, argv[2]);
